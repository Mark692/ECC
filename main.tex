...
\\
Un sistema proposto verso la fine del 19$^\circ$ secolo, da C. Wheatstone e adottato nella Seconda Guerra Mondiale, consisteva nel disporre i simboli dell'alfabeto in una matrice (quadrato di Polibio) in cui:
\begin{enumerate}[label=\alph*)]
 \item i caratteri vengono presi a coppie
 \item per ogni coppia di caratteri vengono tracciate le righe e colonne corrispondenti
 \item la coppia di caratteri viene cifrata con il carattere evidenziato dall'intersezioni di righe/colonne
 \item i caratteri che si trovano sulla medesima linea (riga o colonna) vengono sostituiti dai caratteri immediatamente seguenti
 \item caratteri uguali vengono rappresentati come un unico carattere
\end{enumerate}
...
\begin{enumerate}
 ...
 \end{center}
\end{enumerate}
Solo, ed esclusivamente, con la chiave posseduta dai due interlocutori si pu\`o decifrare un messaggio cifrato. Il vantaggio dell'algoritmo \`e l'impiego di una sola chiave $K^S$ avente lunghezza in bit ridotta, segue un'elevata velocit\`a nella sua generazione e nelle operazioni di cifratura e decifratura, con la diretta conseguenza di poter esser adoperato su documenti di grossa dimensione.
\\
Lo svantaggio \`e rappresentato dallo \textbf{scambio}: sebbene Alice e Bob debbano concordare la chiave, non si pu\`o affidare al mezzo insicuro lo scambio di tale informazione.
\\
...
Una \textbf{curva} nel linguaggio matematico \`e sinonimo di linea. Si parla non di curva in senso assoluto, ma di tipi di curve (come curva continua, curva piana, curva liscia), ciascuno di questi \`e rigorosamente definito.
\\
Una curva si pu\`o rappresentare analiticamente in varie maniere qualora si introduca un sistema di coordinate nello spazio in cui essa \`e immersa: sar\`a allora rappresentata da equazioni soddisfatte dalle coordinate di tutti i punti, e solo questi, della curva stessa \cite{curva3c}.
\\
...
\\
La curva ellittica nel piano proiettivo ha un singolo punto all'infinito $\mathcal{O}$ avente coordinate omogenee $[0:1:0]$, tale punto costituisce l'elemento identit\`a del gruppo. Inoltre la curva \`e simmetrica rispetto all'asse $x$. Per ogni punto $P$ possiamo trovare $-P$ sulla curva al punto opposto rispetto all'asse di simmetria. Per convenzione il punto all'infinito $-\mathcal{O}$ viene considerato semplicemente come $\mathcal{O}$.\\
\`E possibile affermare che le curve ellittiche formano un Gruppo date le seguenti affermazioni:
\begin{enumerate}
 \item l'operazione ``+" lega due punti della curva ad un terzo ancora appartenente alla curva. L'operazione cos\`i definita gode della propriet\`a della ``chiusura" e risulta valida per esser definita Legge di Gruppo;
 ...
 \item la legge di gruppo ``somma" \`e definita come ``dati tre punti $A$, $B$ e $C$ della curva, diversi da $\mathcal{O}$, la loro somma \`e $A+B+C=\mathcal{O}$.
\end{enumerate}
Quest'ultima affermazione, valida per tre punti qualsiasi della curva, ci permette di affermare che:
\begin{center}
$A+(B+C)=B+(A+C)=C+(A+B)=C+(B+A)=\cdots = \mathcal{O}$
\end{center}
dimostrando che la legge di gruppo riveste la duplice qualit\`a di essere sia associativa che commutativa. Il gruppo definito da una curva ellittica \`e quindi un \textit{Gruppo Additivo Abeliano}.
\begin{figure}[H]
...
Usiamo il termine ``Point Addition" per far riferimento alla somma ``+" di due punti sulla curva, questa operazione costituisce la Legge di gruppo per le curve ellittiche. Definiamo questa operazione come: ``Dati tre punti distinti, $K$-razionali, $A$, $B$ e $C$ sulla curva, la loro somma \`e 
\begin{center}
 $A+B+C = \mathcal{O}$
\end{center}
ovvero l'identit\`a". Per definizione di ``elemento identit\`a" possiamo scrivere 
\begin{center}
 $A+B = \mathcal{O} -C = -C$
\end{center}
Il terzo punto $C(x_C, y_C)$ \`e dato dall'intersezione della retta passante per $A$ e $B$ con la curva. Il suo simmetrico $-C(x_C, -y_C)$ rappresenta il risultato della legge di gruppo applicata a due punti $A$ e $B$. 
\\
...
\begin{enumerate}
 ...
 \\
 Tramite il metodo della Point Addition si costruisce una retta per i due punti che risulta parallela all'asse y che andr\`a ad intersecare il punto all'infinito $\mathcal{O}$. Definiamo quindi $P + Q = P+(-P) = \mathcal{O}$ (fig. \ref{PA_P+(-P)});
 \begin{figure}[H]
 ...
 Ricordando che il punto $\mathcal{O}$ viene trattato come elemento identit\`a del gruppo possiamo scrivere $P+\mathcal{O} = \mathcal{O}+P = P$. Si consideri, a tal proposito, la stessa figura (\ref{PA_P+(-P)}) del caso precedente;
...
\end{enumerate}
Possiamo ora determinare come calcolare il punto $C(x_C, y_C) = -(A + B)$ per ogni coppia di punti $A$, $B$ sulla curva. Passiamo allora alle formule. Per semplice costruzione geometrica, la retta $r_{AB}$ passante per i punti $A(x_A, y_A)$ e $B(x_B, y_B)$ presenter\`a un coefficiente angolare $m$ ed una quota $q$; l'intersezione della retta con la curva fornisce le coordinate del punto simmetrico a quello cercato. Le formule per determinare direttamente la coppia $(x_C, y_C)$ sono:
\begin{align*}
\begin{cases}
...
\begin{center}$
    ...
    \end{cases}
     \iff (mx+q)^2 = x^3 + ax+b$
\end{center}
Possiamo allora portare tutto nello stesso membro dell'equazione ed uguagliare il risultato a zero. Svolgendo i conti otteniamo un'equazione di terzo grado nell'incognita $x$. Eguagliamo infine tale equazione al prodotto delle radici determinate delle intersezioni nelle ascisse $x_A$, $x_B$ ed $x_C$:
\begin{align*}
 x^3 + ax+b - (mx+q)^2 &= 0\\
 ...
 &= (x-x_A)(x-x_B)(x-x_C)
\end{align*}
Svolgendo il termine destro ed uguagliando i coefficienti legati ad $x$ dello stesso grado otteniamo le uguaglianze:
\begin{center}
$\begin{cases}
 x^3 = x^3\\
 -m^2x^2 = -(x_A+x_B+x_C)x^2\\
...
%
%
Nel caso particolare (\ref{PointDoubling}) si \`e parlato della somma di un punto $A$ e se stesso, quando questo non \`e un flesso: denominiamo tale caso come ``\textit{Point Doubling}". Le equazioni viste per la legge di gruppo tuttavia non permettono di calcolare il punto $2A$: bisogna trovare delle formule che tengano conto della \textbf{tangente} alla curva $E$ nel punto $A$. La retta tangente $y = mx+q$ presenter\`a un coefficiente angolare $m$ dato dal gradiente dell'equazione (\ref{Forma Breve}):
\begin{align*}
...
m &= \ddfrac{3x^2 + a}{2y}
\end{align*}
Si noti come il coefficiente angolare $m$ dipenda non solo dalle coordinate del punto $A$ ma anche dal coefficiente ``$a$" presente nell'equazione della curva.
\\
Definiti quindi i punti $A = (x_A, y_A)$ e $C=(x_C, y_C) = -2A$, le operazioni per effettuare una Point Doubling sono:
\begin{gather}
\begin{cases}
...
\end{cases}
\end{gather}
Le coordinate di $C$ sono calcolate mediante formule analoghe alla Point Addition: si considera l'intersezione della tangente con la curva $E$, si eguaglia il risultato al prodotto delle radici $x_A$ e $x_C$ ricordando che la prima ha molteplicit\`a 2. Tale operazione consiste nel considerare le precedenti coordinate $(x_B, y_B)$ come se fossero $(x_A, y_A)$. Infine si considerano i monomi dello stesso grado, esattamente come fatto per la Legge di gruppo, trovando le formule qui sopra descritte.
\section{Moltiplicazione Scalare}
Calcolare $nP$, con $n$ un numero intero, su una curva ellittica costituisce l'operazione detta ``\textit{moltiplicazione scalare}" o Point Multiplication. Nelle applicazioni crittografiche vengono richieste moltiplicazioni scalari con coefficienti molto elevati, applicare numerose volte la Legge di gruppo non fornisce un'elevata efficienza nei calcoli. Allo scopo, supponiamo di voler calcolare il punto $100P$ ed illustriamo alcune tecniche utilizzate in pratica.
\\
...
\end{tabular}
\end{center}
Abbiamo raggiunto il nostro scopo in un totale di 8 passi, un risultato nettamente migliore degli $n-1$ visti precedentemente. Applicare questo algoritmo su di una curva ellittica significa nel computare una Point Addition per ogni \textit{Add} ed una Point Doubling per ogni \textit{Double}. 
\\
...
\end{figure} 
Si noti che ora \textbf{l'asse di simmetria} non risulta pi\`u essere $y=0$ ma \textbf{diventa} $\mathbold{y=p\big / 2}$. 
\\
...
in questo caso abbiamo sempre cardinalit\`a \textbf{pari}. Anche in questo caso evidenziamo una generalizzazione: la (\ref{CPcard_pari}) vale per ogni coppia di punti $(n-k)G$ e $(n+k)G$ di medesima coordinata $x_{n-k} = x_{n+k}$. La distanza tra i due punti corrisponde a calcolare $2k$ Point Addition".
 ...
 \\
 La Legge di gruppo ci permette di dire che $Q-Q = \mathcal{O} = \#H \cdot Q$ in quanto simmetrici ed appartenenti ad un campo modulare. Dette $n$ e $m$ le loro molteplicit\`a tali per cui $Q = nG$ e $-Q = mG$, la considerazione precedente ci porta ad affermare che $n+m = \#H$. La conclusione alla quale arriviamo \`e che
 \begin{gather}
...
\end{center}
si sarebbe identificato quindi un sottogruppo con il solo elemento neutro della Legge di gruppo e nessun altro punto.
\\
...
\section{ECDH}
\label{ecdh}
L'algoritmo ECDH \`e l'acronimo di \textbf{E}lliptic \textbf{C}urve \textbf{D}iffie-\textbf{H}ellman e si basa sull'accordo di una chiave. Dette Alice e Bob le parti in gioco, l'algoritmo si basa sui seguenti passi:
\begin{enumerate}
...
\item $d_A$ = hex(random(1, n-1)) = $0x$CA26D640 64C04556 8BC087B7 73137390 D2D412D8 E85E3BBA 11E7A87A F02A460C,

\item $P_A = (x_A, y_A) = d_AG$,

\item $x_A$ = $0x$50F1D077 DE0807EB 17E45781 BAD7AC8A 85EFBE48 5DB3988F 546CF300 E50F4750,

\item $y_A$ = $0x$C5C0324A F29FE9CF 4FC0E305 EE406E4D CF1D6166 9877CA62 B16ADE01 2466C0C7;
\end{itemize}
Bob invece calcola:
\begin{itemize}
\item $d_B$ = hex(random(1, n-1)) = $0x$8840D8F4 08C63786 693A58C3 431489FE 6452C91E 2BD85B5A A982EC04 82B7D965,

\item $P_B = (x_B, y_B) = d_BG$,

\item $x_B$ = $0x$D9852BD0 8856BF07 1FF719E4 2B8D4CB1 E82D66F5 B0CB00CF C6B62892 E1A8FFCD,

\item $y_B$ = $0x$C28B6FCA 34E247C8 32EE7C4F 55EC1DD5 150E6800 66BDFD86 1A8A26A7 724CE7E9.
\end{itemize}
Il segreto condiviso sar\`a il punto di coordinate: $S = (x_S, y_S)$,
\begin{itemize}
    \item $x_S$ = $0x$11F9892D 57C6EAAD B9DE6475 BBCA2204 01FEAA84 3B7A1DB1 3C44C348 C0FB92F1,
    \item $y_S$ = $0x$89FC9ED0 FF4B1D91 3E4978EF DF23BAD8 947528E0 C4DDD00D C0119149 DD5C08ED
\end{itemize}
...
\begin{itemize}
 ...
 \item \textbf{z} stringa corrispondente ai primi $\mu$ bit pi\`u a sinistra di $H$, tuttavia deve esser convertita in un numero intero prima di poter essere utilizzata,
 \begin{itemize}
  \item $\mathbold{\mu} = min(N, h)$ \`e il valore minimo tra $N$ ed $h$,
  \item dato il messaggio $M$, l'impronta $H = hash(M)$ ha una lunghezza di $h$ bit.
 \end{itemize}
\end{itemize}
...
%
%
\chapter{Elliptic Curve Discrete Logarithm Problem}
\label{ecdlp}
Nella sicurezza informatica dire che un problema \`e difficile da risolvere significa che algoritmi e protocolli basati su di esso risultano crittograficamente pi\`u sicuri di altri che fanno uso di procedure semplici o prevedibili. Gli algoritmi analizzati nel capitolo \ref{ecc algo} sono costruiti sulla base di problemi DLP, ognuno con lo scopo di proteggere dati sensibili da utenti malintenzionati.
\\
Sappiamo che il problema del logaritmo discreto (DLP) viene descritto a partire dalla generica equazione
\begin{center}
 $y = a^x$ mod$(n)$
\end{center}
...
\end{center}
Per ogni coppia di $s$ calcoliamo il corrispondente punto $aP+bQ$. Chiamata $S$ la sequenza data dai punti $R_i = a_iP+b_iQ$, la sua struttura estesa \`e:
\begin{center}
 $S=\{R_1, R_2, \ldots, R_j\}$
\end{center}
...
\\
La $\rho$ di Pollard prosegue mediante algoritmi di ricerca dei cicli. Uno di questi \`e l'algoritmo di Floyd, ``\textit{la Lepre e la Tartaruga}": data la sequenza $S$ leggiamo ogni suo punto ma a due velocit\`a differenti. Chiamiamo $T$ la tartaruga, questa ha il compito di leggere ogni singolo punto; chiamiamo invece $L$ la lepre che ha il compito di leggere i punti $R_i$ alternati. Confrontando i punti trovati da $T$ ed $L$ ad ogni loro passo arriviamo a trovare due punti $R_T$ e $R_L$ con le stesse coordinate ma generati da diverse coppie di $s$. Terminiamo l'algoritmo con il calcolo del segreto $k$ per mezzo della $k=(a -A)(B -b)^{-1}$ mod$(n)$.
\\
...
\begin{center}
 ...
 \label{p parallelo}
\end{center}
Una sua versione parallelizzata su 2600 computer \`e stata utilizzata da Chris Monico nel 2004 \cite{monico} per una sfida lanciata dalla Certicom. La sfida consisteva nel riuscire a trovare il segreto $k$ data una curva ellittica di lunghezza pari a $109$ bit. L'ordine di tale curva corrisponde quindi a $n = 2^{109}$. La versione dell'algoritmo proposta da Monico ha richiesto 17 mesi di computazioni.
\\
...
\begin{center}
 $T = t \cdot \ddfrac{\sqrt{\ddfrac{\pi 2^B}{2}}}{\sqrt{\ddfrac{\pi n}{2}}} = t \cdot \ddfrac{\sqrt{2^{B}}}{\sqrt{2^{109}}}$
\end{center}
...
\\
L'algoritmo parallelizzato con $M$ processori comporta un miglioramento di sole $\sqrt{M}$ operazioni%http://people.scs.carleton.ca/~paulv/papers/JoC97.pdf
. La spiegazione deriva dal fatto che non salvando i punti in una hash table, ogni processore dovr\`a calcolare una propria sequenza di punti. Ogni processore avr\`a la medesima probabilit\`a degli altri $M-1$ di trovare una collisione. Per diminuire linearmente i passi da computare bisogna assegnare ad ogni processore la medesima sequenza ma facendoli partire da punti iniziali $P_M$ diversi. 
\\
...
%
%
\section{La \texorpdfstring{$\mathbold{\lambda}$}{l} di Pollard}
...
\end{align*}
\end{center}
La distanza di salto $d$ viene generalmente identificata \cite{el-psy 2} da un elemento dell'insieme finito $S$ delle potenze di 2 $S=\{2^0, 2^1, \ldots, 2^M\}$. La scelta di un elemento $d$ di $S$ \`e casuale ad ogni salto (iterazione).
\\
...
\end{align*}
Entrambi rispecchiano l'analoga versione della $\rho$ di Pollard, tuttavia bisogna conoscere un intervallo molto ristretto di valori nel quale cercare $k$ ed \`e possibile dover ripetere l'algoritmo se la scelta di $w_0$ non porti a soluzione, aumentando notevolmente il tempo di riuscita. Per questi motivi l'algoritmo della $\rho$ viene ritenuto pi\`u efficiente della $\lambda$.
%
%
%
%
...
In questo capitolo abbiamo visto diversi attacchi che mirano a risolvere l'ECDLP. Ogni algoritmo si fonda su principi matematici differenti o su curve ellittiche aventi specifiche propriet\`a. Il BSGS non presenta facile applicazione per via del suo costo spaziale; l'algoritmo di Poligh-Hellman richiede curve supersingolari, non trovando alcuna applicazione tra gli standard del NIST: tutte le curve promosse sono curve prime, il cui minimo ordine \`e $n=192$. Test di primalit\`a si trovano nello standard \cite{dss} a dimostrazione del fatto che le curve utilizzate sono assolutamente sicure contro questo tipo di attacco. 
\\
Una seconda nota da evidenziare \`e che tali curve hanno cardinalit\`a $\#E$ diversa dalla caratteristica di campo $p$; non possono esser definite come Curve Anomale. Esistono algoritmi per la risoluzione dell'ECDLP su curve anomale ma, come affermato dallo stesso ideatore Nigel Smart in \cite{ev_curv2}, si tratta di metodi di interesse teorico piuttosto che pratico. Gli algoritmi di Pollard restano i pi\`u veloci a nostra disposizione sebbene non presentino un tempo di calcolo sub-esponenziale, rendendoli inefficaci contro le curve di grandi ordini $n$.
\\
\`E possibile pensare di velocizzare questi algoritmi o aumentarne le probabilit\`a di riuscita facendo affidamento alle formule (\ref{CPcard_dispari}) e (\ref{Cons Pers 2}) mediante le quali risolviamo l'ECDLP anche quando arriviamo a calcolare $-Q$, il punto simmetrico alla nostra chiave pubblica. Gli algoritmi di Pollard avrebbero due punti sui quali fermarsi raddoppiando le probabilit\`a di riuscita. L'algoritmo di Poligh-Hellman non viene influenzato da queste formule, infatti non viene modificato l'ordine $n$ sul quale esso si basa.
\\
...
Dall'analisi condotta nel capitolo \ref{ecdlp} si \`e arrivati a mostrare come gli algoritmi pi\`u veloci per risolvere l'ECDLP siano gli algoritmi di Pollard. Volendo risolvere il problema dell'ECDLP, se l'algoritmo pi\`u veloce a noi noto richiede $2^k$ operazioni, diciamo che il problema presenta un \textbf{livello di sicurezza }$\mathbold{k}$. 
\\
...
\\
Prendiamo in considerazione anche il tempo di elaborazione dell'algoritmo: un anno MIPS corrisponde al numero di operazioni che vengono svolte in 365 giorni di attivit\`a da un singolo processore che lavora alla frequenza di 1 milione di operazioni al secondo. Tale numero corrisponde a $MIPS=3.1 \cdot 10^{13}$ operazioni totali. Gli anni MIPS necessari per il calcolo $x$ operazioni sono calcolati come il rapporto $x / MIPS$, vedasi le colonne \textit{MIPS RSA} e \textit{MIPS ECC}. Infine, nella colonna \textit{MIPS ECC/RSA} mostriamo un rapporto tra gli anni MIPS necessari ad entrambi gli algoritmi per risolvere il logaritmo discreto.
\begin{center}
\begin{tabular}{c |c |c |c |c |c |c }
& \multicolumn{3}{c|}{Operazioni} &\multicolumn{3}{c}{Anni MIPS}\\
\hline
L & RSA & ECC & ECC/RSA & RSA & ECC & ECC/RSA\\
\hline
80 & $3.8 \cdot 10^{13}$ & $1.5 \cdot 10^{24}$ & $3.9 \cdot 10^{10}$ &  1.2 & $4.7 \cdot 10^{10}$ & $3.9 \cdot 10^{10}$ \\
112 & $1.9 \cdot 10^{18}$ & $6.5 \cdot 10^{33}$ & $3.4 \cdot 10^{15}$ &  $6.0 \cdot 10^{4}$ & $2.0 \cdot 10^{20}$ & $3.4 \cdot 10^{15}$ \\
128 & $5.2 \cdot 10^{21}$ & $4.3 \cdot 10^{38}$ & $8.2 \cdot 10^{16}$ &  $1.6 \cdot 10^{8}$ & $1.3 \cdot 10^{25}$ & $8.2 \cdot 10^{16}$ \\
192 & $6.0 \cdot 10^{31}$ & $7.9 \cdot 10^{57}$ & $1.3 \cdot 10^{26}$ &  $1.9 \cdot 10^{18}$ & $2.5 \cdot 10^{44}$ & $1.3 \cdot 10^{26}$ \\
256 & $1.4 \cdot 10^{42}$ & $1.5 \cdot 10^{77}$ & $1.0 \cdot 10^{35}$ &  $4.4 \cdot 10^{28}$ & $4.7 \cdot 10^{63}$ & $1.0 \cdot 10^{35}$
\end{tabular}
\end{center}
...
\\
A confronto, chiavi pi\`u piccole per ECC consentono di velocizzare notevolmente i calcoli di generazione per la chiave privata: si fa infatti uso di un numero casuale $d$ scelto in un ampissimo intervallo. Ricordando che la chiave privata $d$ per algoritmi ellittici va scelta nell'intervallo $[1, n-1]$ diamo un'idea di quanto ampia sia la scelta. Nello standard \cite{dss} la NIST raccomanda l'utilizzo di cinque curve diverse, ognuna per un differente livello di sicurezza, per generare la firma digitale mediante ECDSA. Tali curve sono denominate $P_{x}$ dove $x$ determina la pi\`u grande potenza di 2 utilizzata nell'esprimere la cardinalit\`a del campo primo $\mathbb{F}_p$:
\begin{align*}
...
\end{center}
e conseguente scelta della chiave privata nell'intervallo $[1, 6.2\cdot 10^{58}]$.
\\
\\
